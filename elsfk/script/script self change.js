...
function checkForCollision() {
    ...
    for (let row = 0; row < shapeHeight; row++) {
        ...
        if (shape[row % shapeHeight][col % shapeWidth] === 1) { // 如果是方块
          ...
          // 如果该位置已经有方块存在
            if (((fallenBlocks[gameRow] && fallenBlocks[gameRow][gameCol]) !== null )&& ((fallenBlocks[gameRow] && fallenBlocks[gameRow][gameCol]) !== undefined)) {
            return true;
            }
        }
        }
